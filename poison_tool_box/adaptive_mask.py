...
class poison_generator():
    ...
    def __init__(self, img_size, dataset, poison_rate, path, trigger, target_class=0, alpha=0.2, cover_rate=0.01, pieces=16, mask_rate=0.5):
    ...
    def generate_poisoned_training_set(self):
        ...
        for i in range(self.num_img):
            ...
            if pt < num_poison and poison_indices[pt] == i:
                ...
                mask = get_trigger_mask(self.img_size, self.pieces, self.masked_pieces)
                # print(mask.shape, img.shape, self.trigger.shape)
                img = img + self.alpha * mask * (self.trigger - img)
                pt+=1
